...
fetch('http://localhost:5000/staticvalues').then(resp => resp.json()).then(data => {
    console.log(`Connecting socket => http://${data.HOST}:${data.WEBSOCKET_PORT}`);
    staticValues = data;
...
function connect(){
    socket = io(`http://${staticValues.WEBSOCKET_HOST}:${staticValues.WEBSOCKET_PORT}`);
